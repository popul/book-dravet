...
```mermaid
graph TD
    A[Dialogue direct<br/>referent / chef de service] --> B{Resolu ?}
    B -->|Oui| C[Documenter<br/>la solution]
    B -->|Non| D[Demande ecrite<br/>au directeur]
    D --> E{Resolu ?}
    E -->|Oui| C
    E -->|Non| F[Personne qualifiee<br/>mediateur gratuit]
    F --> G{Resolu ?}
    G -->|Oui| C
    G -->|Non| H[Saisine du CVS]
    H --> I{Resolu ?}
    I -->|Oui| C
    I -->|Non| J[Signalement<br/>ARS Occitanie]
    J --> K{Resolu ?}
    K -->|Oui| C
    K -->|Non| L[Defenseur<br/>des droits]
    L --> M{Resolu ?}
    M -->|Oui| C
    M -->|Non| N[Tribunal<br/>administratif]
```
