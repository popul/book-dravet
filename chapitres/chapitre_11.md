...
```mermaid
graph TD
    A["Diagnostic"] --> B["Certificat médical<br/>détaillé"]
    B --> C["Dossier MDPH"]
    C --> D{"Commission<br/>d'évaluation"}
    
    D -- Scolarité --> E["PPS / AESH"]
    D -- Social --> F["AAH / PCH"]
    D -- Protection --> G["Tutelle /<br/>Curatelle"]
...
```mermaid
graph TD
    A["Évaluation<br/>des besoins"] --> B{"Niveau de<br/>surveillance ?"}
    
    B -- "Autonomie<br/>partielle" --> C["Foyer de vie"]
    B -- "Surveillance<br/>médicale" --> D["FAM"]
    B -- "Soins<br/>constants" --> E["MAS"]

    C --> F["Protocole<br/>d'urgence"]
    D --> F
    E --> F

    F --> G["Monitoring<br/>nocturne"]
    G --> H["Révision<br/>annuelle"]
