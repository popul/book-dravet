...
```mermaid
graph TD
    A["Début de crise"] --> B{"Durée < 5 min ?"}
    B -- Oui --> C["Surveillance<br/>Protection, PLS"]
    B -- Non --> D["URGENCE"]
    
    D --> E["Médicament de secours<br/>(Midazolam/Diazépam)"]
    E --> F{"Arrêt ?"}
    
    F -- Oui --> G["Surveillance<br/>post-critique"]
    F -- Non --> H["Appel SAMU (15)"]
    H --> I["Transfert hôpital"]
